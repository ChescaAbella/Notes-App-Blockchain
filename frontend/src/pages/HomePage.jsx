...
const NoteCard = memo(({ note, onClick, onTogglePin, onToggleFavorite, onDelete }) => (
  <div className={`note-item ${note.is_pinned ? 'pinned' : ''} ${note.deleted_at ? 'deleted' : ''}`} style={{cursor: 'pointer'}}>
...
export default function HomePage() {
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      console.log("Attempting to delete note:", { id: noteToDelete.id, txHash: deletionTxHash });
      
      // Save deletion to backend with blockchain tx hash
      const response = await api.post(`notes/${noteToDelete.id}/soft-delete`, { txHash: deletionTxHash });
      console.log("Delete response:", response.data);
      
      // Update note in UI with deleted_at timestamp
      ...
      showToast("Note soft deleted on blockchain! Check trash to restore.", "success");
  ...
  const handleRestoreNote = useCallback(async (note) => {
    try {
      ...
      // Update note in UI - remove deleted_at timestamp
  ...
  // Memoized filtered notes with filter and search - excludes deleted notes unless in trash view
  const filteredNotes = useMemo(() => {
    let filtered = notes;

    // If in trash view, show only deleted notes; otherwise exclude deleted notes
    if (showTrash) {
    ...
    return filtered
      .filter((note) => {
        ...
        // Only apply category filters when not in trash
      ...
      .sort((a, b) => {
        // Sort by pinned status first (pinned notes come first) - only if not in trash
        if (!showTrash) {
          if (a.is_pinned && !b.is_pinned) return -1;
          if (!a.is_pinned && b.is_pinned) return 1;
        }
        // Then sort by timestamp (or deleted_at if in trash)
